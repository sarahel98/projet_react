...
const Cards = (apiData) => {
 // Fonction qui permet d'ajouter la météo de la ville sur la page de favoris et d'ajouter son id dans le local storage 
  const handleAddFav = (data) => {
    ...
    let weatherFavs = []
    const store = JSON.parse((localStorage.getItem('weatherFavs')))
    if (store !== null) {
      weatherFavs = store;
    }
    if (!weatherFavs.includes(data.id)) {
      localStorage.setItem("weatherFavs", JSON.stringify([...weatherFavs, data.id]));
    }
    console.log(localStorage.getItem('weatherFavs'))
